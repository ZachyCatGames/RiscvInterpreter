...
template<typename T>
constexpr auto GenerateMaskRight(std::size_t size) noexcept {
    return std::numeric_limits<T>::max() >> ((sizeof(T) * 8) - size);
}
...
template<typename T>
constexpr auto GenerateMaskLeft(std::size_t size) noexcept {
    return std::numeric_limits<T>::max() << ((sizeof(T) * 8) - size);
}
